...
@section('content')
    <div class="bg-tan-200">
        <div class="mx-auto max-w-2xl px-6 py-8">
            <div class="rounded-2xl border-2 border-tan-400 bg-tan-300 p-6 shadow-sm ring-1 ring-black/5">
                <h1 class="text-xl font-bold text-black">Reservering betalen</h1>

                <div class="mt-4 rounded-lg border border-olivegreen-600 bg-olivegreen-300 px-4 py-3 text-sm text-black space-y-1">
                ...
                <form method="POST" action="{{ $checkoutUrl }}" class="mt-6">
                    @csrf
                    <button type="submit" class="w-full rounded-lg bg-cerulean-300 border-2 border-cerulean-400 py-2 text-sm font-semibold text-white transition hover:bg-cerulean-400">
                        Betalen met Stripe
                    </button>
                </form>
