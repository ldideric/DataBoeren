...
<dl class="space-y-2 text-sm text-white">
    <div class="flex justify-between">
        ...
        <dd class="font-medium text-white">{{ $euro($order->nightly_rate * $order->num_nights) }}</dd>
    </div>

    <div class="flex justify-between">
        <dt>Volwassenen · {{ $euro($order->per_adult_rate) }} × {{ $adults }} × {{ $order->num_nights }}</dt>
        <dd class="font-medium text-white">{{ $euro($order->per_adult_rate * $adults * $order->num_nights) }}</dd>
    </div>

    @if ($children > 0)
        <div class="flex justify-between">
            <dt>Kinderen · {{ $euro($order->per_child_rate) }} × {{ $children }} × {{ $order->num_nights }}</dt>
            <dd class="font-medium text-white">{{ $euro($order->per_child_rate * $children * $order->num_nights) }}</dd>
        </div>
    @endif

    @if ($order->last_minute_applied && $order->last_minute_discount)
        <div class="flex justify-between pt-3 font-semibold text-white">
    ...
    @if ($order->extras_total > 0)
        <div class="flex justify-between">
            <dt>Extra's</dt>
            <dd class="font-medium text-white">{{ $euro($order->extras_total) }}</dd>
        </div>
    @endif

    <div class="flex justify-between border-t border-gray-200 pt-2 text-base">
        <dt class="font-semibold text-white">Totaal</dt>
        <dd class="font-bold text-white" data-grand-total data-base="{{ $order->total }}" data-nights="{{ $order->num_nights }}">{{ $euro($order->total) }}</dd>
    </div>
</dl>
