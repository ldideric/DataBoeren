...
@section('content')
    <div class="bg-tan2">
        <div class="mx-auto flex min-h-[60vh] max-w-2xl items-center justify-center px-6 py-10">
            <div class="w-full max-w-md rounded-2xl bg-tan p-6 text-center shadow-sm ring-1 ring-black/5">
                <h1 class="text-xl font-semibold text-black">Controleer uw e-mail</h1>

                @if (session('status'))
                    <div class="mt-4 rounded-md border border-cerulean bg-cerulean px-4 py-2 text-sm text-white">
                        {{ session('status') }}
                    </div>
