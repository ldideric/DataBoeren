...
it('lists available campsites that fit the party once searched', function () {
    Campsite::factory()->create(['name' => 'Grote Plek', 'max_people' => 6]);
...
it('filters by accommodation type in SQL, not just in the browser (bug: filters work for all fields)', function () {
    Campsite::factory()->ofType(CampsiteType::Koeienveld)->create(['name' => 'Koe Plek', 'max_people' => 6]);
    Campsite::factory()->ofType(CampsiteType::Geitenveld)->create(['name' => 'Geit Plek', 'max_people' => 6]);
...
it('filters by multiple accommodation types at once', function () {
    Campsite::factory()->ofType(CampsiteType::Koeienveld)->create(['name' => 'Koe Plek', 'max_people' => 6]);
    Campsite::factory()->ofType(CampsiteType::Geitenveld)->create(['name' => 'Geit Plek', 'max_people' => 6]);
    Campsite::factory()->ofType(CampsiteType::Schapenveld)->create(['name' => 'Schaap Plek', 'max_people' => 6]);
...
it('keeps the type filter when paging (bug: page 2 reset the filter)', function () {
    Campsite::factory()->count(12)->ofType(CampsiteType::Koeienveld)->create(['max_people' => 6]);
    Campsite::factory()->count(5)->ofType(CampsiteType::Geitenveld)->create(['max_people' => 6]);
...
it('searches live as soon as both dates are present (no submit needed)', function () {
    Campsite::factory()->create(['name' => 'Live Plek', 'max_people' => 6]);
